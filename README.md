...
- **역할**: **팀원**
- **구현 기능**
	- 마이페이지(프로필, 후기 관리, 게시글 관리, 댓글 관리)
	- 1:1 상담(채팅), 주문 내역 관리(비동기 결제 취소, 반품 신청, 비동기 후기 작성 등)
	- 이미지 삭제 스케줄러, 오라클 클라우드 호스팅
- **기여도**: 설계 단계에서 DB 설계에 적극적으로 참여, 주도 하였고, 맡은 기능들을 정해진 기간 내에 모두 구현하였습니다. 또 프로젝트 제작 완료 후 오라클 클라우드에 호스팅을 맡아 진행하였습니다.
